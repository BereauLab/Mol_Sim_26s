"""
Minimal Lennard-Jones molecular dynamics with Velocity Verlet integration.
...
def write_energy(fh, step, ke, pe, T):
    """Append a line of step KE PE E_tot T to an energy log."""
    fh.write(f"{step:>8d} {ke:>14.6f} {pe:>14.6f} {ke + pe:>14.6f} {T:>10.6f}\n")


def run(n_part=200,box_size=8.0,r_cut=4.0,temperature=1.0, dt=0.005,
        eq_steps=None, prod_steps=2000,write_every=20, traj_save=True,
        traj_file_name=None,ene_save=True, ene_file_name=None, seed=42):
    ...
    if ene_save:
        ...
        fh_ene.write(f"# {'step':>6} {'KE':>14} {'PE':>14} {'E_tot':>14} {'T':>10}\n")
    else:
        fh_ene = None
    ...
    for step in range(prod_steps + 1):
        ...
        if step % write_every == 0:
           ...
           if fh_ene is not None:
               write_energy(fh_ene, step, ke, pe, T)
...
if __name__ == "__main__":
    ...
    prod_steps = int(4e3) # Number of production steps
    seed = 42 # Random seed
    write_every = 10 # Control every how many steps it saves energy and positions
    r_cut = 2.5 # Cutoff 
    traj_file_name = 'lj_traj.xyz' # Name of the file to which the trajectory would be written
    ene_file_name = 'lj_energy.dat' # Name of the file to which the energies would be written
    ...
    run(n_part=n_part, box_size=box_size, r_cut=r_cut, temperature=temperature,
        ...
        ene_save=True, ene_file_name=ene_file_name, seed=seed)
